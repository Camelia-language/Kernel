parser grammar CameliaParser;

options {
  output = AST; // Produce a tree of node
  tokenVocab = CameliaLexer; // Use tokens defined in our lexer.
...
literal returns [Node node]:
  ...
  | NIL               { $node = new LiteralNode(CameliaRuntime.getNil()); }
  | TRUE              { $node = new LiteralNode(CameliaRuntime.getTrue()); }
  | FALSE             { $node = new LiteralNode(CameliaRuntime.getFalse()); }
